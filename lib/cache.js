...
export const PROMPT_VERSION = "ai-terms-v2";
...
export function makeSegmentKey(segment, model) {
  return stableHash(`${PROMPT_VERSION}\u0000${model}\u0000${segment.kind || "body"}\u0000${segment.text}`);
}
...
export function readCachedItems(cache, { url, segments, model }, now = Date.now()) {
  ...
  const items = segments.flatMap((segment) => {
    const entry = article.entries?.[makeSegmentKey(segment, model)];
    return entry ? [{ id: String(segment.id), translation: entry.translation, terms: entry.terms || [] }] : [];
  });
  return { cache: clean, items };
}

export function mergeCachedItems(cache, { url, segments, items, model }, now = Date.now()) {
  const clean = pruneCache(cache, now);
  ...
  for (const item of items) {
    ...
    entries[makeSegmentKey(segment, model)] = {
      translation: item.translation,
      terms: Array.isArray(item.terms) ? item.terms : []
    };
  }
  clean.articles[articleKey] = {
