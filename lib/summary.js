...
const SUMMARY_PROMPT_VERSION = "module-v4";
  ...
  return [
    {
      ...
      content: [
        ...
        "summary 写一到三句中文，最多 150 个汉字：第一句直接给出本模块最重要的结论，随后只保留理解该结论必需的关键信息。",
        "有重点、好理解：用短句，先结论后补充；删除例子、过程细节、并列论据和背景铺陈；除非数字本身就是核心结论，否则不要列数字。",
...
function compactSummary(value) {
  ...
  let combined = sentences.slice(0, MAX_SUMMARY_SENTENCES).join("");
  if (combined.length <= MAX_SUMMARY_CHARS) return combined;
  while (sentences.length && combined.length > MAX_SUMMARY_CHARS) {
    sentences.pop();
    combined = sentences.join("");
  }
  return combined || text.slice(0, MAX_SUMMARY_CHARS - 1).trimEnd() + "…";
}

export function parseSummaryResponse(raw, fallbackTitle = "") {
  if (typeof raw !== "string" || !raw.trim()) throw new Error("导读服务没有返回内容");
  ...
  if (!title || !summary) throw new Error("导读结果格式不正确");
