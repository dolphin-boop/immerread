...
export function buildTranslationMessages({ title, segments, glossary = {} }) {
  const lockedTerms = Object.entries(glossary)
    ...
    .join("\n");
  const preparedSegments = segments.map(({ id, kind, text, markup }) => ({ id, kind, text, markup }));
  ...
  return [
    {
      ...
      content: [
        ...
        "每个段落可能包含 <strong>、<u>、<em>、<a data-link=\"数字\">、<code>、<br> 标记。",
        "translation 必须翻译标记内外的自然语言并原样保留这些标记、嵌套关系和 data-link；不得新增其他 HTML。",
        "<code> 内的代码或标识符保持原文；<em> 只表示语义强调，不要添加斜体样式。",
        "terms 只收录 AI 和机器学习领域的专有概念，包括模型、训练、推理、评测、智能体和检索增强生成相关术语。",
        "不要把通用编程、软件工程、数据库、浏览器、网络协议或互联网产品词汇放入 terms。",
        "只返回 JSON，不要 Markdown 或解释。",
        '格式：{"items":[{"id":"原 id","translation":"带原标记的中文译文","terms":[{"source":"英文术语","target":"中文术语"}]}]}。',
